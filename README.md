...
| 消息长度 | 传输时间 | 最大频率 | 应用场景 |
|---------|---------|---------|---------|
| 10 字节 | 0.87 ms | **1,150 Hz** | 短消息、状态码 |
| 50 字节 | 4.34 ms | **230 Hz** | 典型 JSON 消息 ⭐ |
| 100 字节 | 8.68 ms | **115 Hz** | 长 JSON 消息 |
| 256 字节 | 22.2 ms | **45 Hz** | 最大缓冲区 |

**软件开销 (双缓冲区系统):**

- 互斥锁获取/释放: ~5-10 μs
- `vsnprintf()` 格式化 (50字节): ~50-100 μs
- DMA 启动: ~2-5 μs
- **总开销**: < 0.1 ms (可忽略)

**性能瓶颈:** UART 硬件传输速度 (115200 bps)

**提升方案:**

1. **提高波特率** (最简单)
   - 230400 bps → 最大 460 Hz (50字节)
   - 460800 bps → 最大 920 Hz (50字节)
   - 921600 bps → 最大 1,840 Hz (50字节)
...
3. **合并多条消息** (减少 DMA 启动次数)

   ```c
   // 2 条消息合并为 1 条
   send_message("{\"NTC\":%.2f,\"PID\":%.2f}\n", temp, pid_out);
   ```
...
- **新增文件**: `VSCODE_CONFIG.md` - VS Code 工作区配置详解
  - 详细说明 `.vscode` 文件夹中各配置文件的作用
  - 提供完整的 `launch.json` 和 `tasks.json` 配置代码（可直接复制）
  - 包含移植步骤、常见问题解答和快速开始指南
  - 适合团队协作和项目移植使用

#### README 文档优化

- 更新项目结构，添加 `.vscode` 文件夹和新文档说明
- 新增 "VS Code 配置" 章节，说明配置文件用途和快捷操作
- 补充开发工具列表（OpenOCD、CMSIS-DAP 等）
- 优化文档排版和结构

#### 配置文件注释优化

- 为 `launch.json` 添加详细的中文注释
  - 每个配置项都有说明和移植提示
  - 包含常用调试器和芯片配置选项列表
  - 添加完整的移植检查清单
- 为 `tasks.json` 添加详细的中文注释
  - 说明每个任务的用途和参数
  - 包含 OpenOCD 和 STM32CubeProgrammer 使用对比
  - 提供移植步骤和常见问题解决方案
